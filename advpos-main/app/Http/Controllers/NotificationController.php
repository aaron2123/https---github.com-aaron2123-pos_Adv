...
class NotificationController extends Controller
{
    ...
    public function sentExpireNotification() {
    
        $trans = DB::table('products')
            ->where('expiry_period','>', 0)
            ->get();
        ...
        {
            if($row->expiry_period_type == 'months')
            {
                $gap = $row->expiry_period;
                $expire_date = (new Carbon($row->created_at))->addMonths($gap);
                $is_for_mail = 1;
            }
            elseif($row->expiry_period_type == 'days')
            {
                $gap = $row->expiry_period;
                $expire_date = (new Carbon($row->created_at))->addDays($gap);
                $is_for_mail = 1;
            }
            
            $today = date("Y-m-d");
            $datetime1 = new DateTime($expire_date);
            $datetime2 = new dateTime($today);
            $interval = $datetime2->diff($datetime1);
            $days = $interval->format('%R%a');
            
            $expiry = date('Y-m-d', strtotime($expire_date));
            
            if($days <= 7 && $days >= 0)
            {
                array_push($product, $row->sku."_".$row->name."_".$row->alert_quantity."_".$days."_".$expiry);
            }
            
            if($days < 0)
            {
                array_push($expired, $row->sku."_".$row->name."_".$row->alert_quantity."_".$days."_".$expiry);
            }
        }
        $count_expiring = 0; $count_expired = 0;
        foreach($product as $row)
        {
            $count_expiring = $count_expiring + 1;
        }
        ...
        //Mail::to(['kageena.arunthavarasa@kodplex.com'])->send(new ExpireNotification($details));
        //Mail::to(['vinoo@kodplex.com'])->send(new ExpireNotification($details));
        //Mail::to(['sangee2230@gmail.com'])->send(new ExpireNotification($details));
        Mail::to(['vinoo.gf@gmail.com'])->send(new ExpireNotification($details));
